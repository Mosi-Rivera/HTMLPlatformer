...
(function()
{
    ...
    class Game
    {
        ...
        constructor(configs)
        {
            ...
            document.getElementsByTagName('body')[0].appendChild(this.drawCanvas.canvas);//modified
            ...
            // body.style.overflow = 'hidden';//modified
            // this.drawCanvas.canvas.style.width = '100vw';//modified
            // this.drawCanvas.canvas.style.height = '100vh';//modified
            if (configs.keyboard)
    ...
    class KeyboardManager
    {
        ...
        constructor(game)
        {
            document.addEventListener('keydown', e => { e.preventDefault(); this.keydown(e.code) });
            document.addEventListener('keyup',e => { e.preventDefault(); this.keyup(e.code) });
            this.game = game;
...
(function(){
    ...
    class Scene extends LIB.Scene
    {
        ...
        collideAt(x,y)
        {
            const collidables = game.collidables;
            for (let i = collidables.length; i--;)
            {
                let {hitbox,position} = collidables[i];
                if (!(
                    x < position.x + hitbox.left ||
                    x > position.x + hitbox.right ||
                    y < position.y + hitbox.top ||
                    y > position.y + hitbox.bottom
                ))
                    return true;
            }
            return false;
    ...
    class Collidable
    {
        ...
        {
            this.position = {x,y};
            this.hitbox = new LIB.Hitbox(w,h,ox,oy);
        }
    }
    ...
    class Game extends LIB.Game
    {
        ...
        constructor(options)
        {
            ...
            let canvas = this.drawCanvas.canvas;
            canvas.style.zIndex = -1;
            canvas.style.position = 'absolute';
            canvas.style.left = '0px';
            canvas.style.top = '0px';
            this.resize(options.collidableClass);
        }
        ...
        resize(collidableClass)
        {
            const canvas = this.drawCanvas;
            canvas.canvas.width = window.innerWidth;
            canvas.canvas.height = html.offsetHeight;
            world_bounds.w = window.innerWidth;
            world_bounds.h = html.offsetHeight;
            canvas.offset.x = 0;
            canvas.offset.y = 0;
            canvas.scale = 1;
            this.drawCanvas.ctx.imageSmoothingEnabled = false;
            if (collidableClass)
            {
                ...
                {
                    let node = nodes[i];
                    this.collidables.push(new Collidable(
                        Math.floor(node.offsetLeft),
                        Math.floor(node.offsetTop),
                        Math.floor(node.offsetWidth),
                        Math.floor(node.offsetHeight),
                        0,
                        0
                    ));
                }
